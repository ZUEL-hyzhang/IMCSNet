...
def gdc(A: sp.csr_matrix, gamma: float, eps: float):
    N = A.shape[0]
    A_loop = sp.eye(N) + A
    ...
    S = gamma * sp.linalg.inv(sp.eye(N) - (1 - gamma) * T_sym)
...
def SIMGCL_aug_feature_dropout(input_feat, sparse_norm_adj):
    all_embeddings = []
    for k in range(4):
        ego_embeddings = torch.sparse.mm(sparse_norm_adj, input_feat)

        random_noise = torch.rand_like(ego_embeddings)
        ego_embeddings += torch.sign(ego_embeddings) * torch.functional.F.normalize(random_noise, dim=-1) * 0.05
        all_embeddings.append(ego_embeddings)
    all_embeddings = torch.stack(all_embeddings, dim=1)
    aug_input_feat = torch.mean(all_embeddings, dim=1)
